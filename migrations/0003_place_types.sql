...
--
-- `icon` is an emoji and is deliberately NULL wherever no emoji honestly means
-- the type: there is none for a via ferrata, a viewpoint, a cave, a lake or a
-- paddleboard, and forcing one in would be worse than none. Nothing in the UI
-- may depend on an icon being there — see docs/features/places.md.

INSERT OR IGNORE INTO entry_type (id, kind, slug, label, icon, colour, position) VALUES
  ('loc_lake',        'location', 'lake',              'Lake',              NULL,  '#2563eb', 10),
  ('loc_viewpoint',   'location', 'viewpoint',         'Viewpoint',         NULL,  '#7c3aed', 20),
  ...
  ('loc_cave',        'location', 'cave',              'Cave',              NULL,  '#44403c', 100),
...
INSERT OR IGNORE INTO entry_type (id, kind, slug, label, icon, colour, position) VALUES
  ...
  ('act_via_ferrata', 'activity', 'via-ferrata',       'Via ferrata',       NULL,  '#ea580c', 20),
  ...
  ('act_sup',         'activity', 'sup',               'Paddleboarding',    NULL,  '#0d9488', 70);
